...
for i in df.index:
	...
	y = scf.ix[i].values
	try:
		p0 = [2*y[-1]-y[-2],1.0,1.0]
		popt,pcov = curve_fit(exp_fit,xdata=n,ydata=y,p0=p0,bounds=([2*y[-2]-y[-3],-np.inf,-np.inf],[y[-1],np.inf,np.inf]))
		scf_extrap.append(popt[0])
	except:
		scf_extrap.append(y[-1])
#	plt.scatter(n,y)
...
scf['numerical'] =[-103.244161, -103.116804, -102.898695, -103.02626, -102.889897, -102.479065, -101.279432, -99.428086, -96.794469, -93.301834, -88.943512, -30.462441, -103.101228]
