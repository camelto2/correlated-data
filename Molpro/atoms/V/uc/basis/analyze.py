...
for i in df.index:
	...
	#p0 = [-0.5,0]
	#popt,pcov = curve_fit(corr_fit1,xdata=n,ydata=y,p0=p0)
	#plt.plot(x,corr_fit1(x,*popt),label='$1/n^3$')
	#p0 = [-0.5,0]
	#popt,pcov = curve_fit(corr_fit2,xdata=n,ydata=y,p0=p0)
	#plt.plot(x,corr_fit2(x,*popt),label='$1/(n+1)^3$')
	#y = cc.ix[i].values-scf.ix[i].values
	p0 = [-0.5,0,0]
	popt,pcov = curve_fit(corr_fit3,xdata=n,ydata=y,p0=p0)
	#plt.plot(x,corr_fit3(x,*popt),label='$1/(n+3/8)^3+1/(n+3/8)^5$')
	#plt.scatter(n,y)
	#plt.plot(x,[2.0*y[1]-y[0]]*len(x),label='$2*qz-tz$')
	#plt.plot(x,[2.0*y[2]-y[1]]*len(x),label='$2*5z-qz$')
