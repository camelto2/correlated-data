...
A_hf = [-948.14091342, -948.14174924, -948.14195113] 
A_cc = [-949.00220331, -949.03401626, -949.05195673] 
B_hf = [-74.85949385, -74.86393635, -74.86466194]
B_cc = [-75.09181489, -75.10916449, -75.11471740]


def extrapolate():
	df=pd.DataFrame()
	for bas in ['tz','qz','5z']:
		x = pd.read_csv(bas+'.table1.txt',skip_blank_lines=True,skipinitialspace=True,comment='#')
		df[bas+'_scf'] = x['SCF']
		df[bas+'_ccsd'] = x['CCSD']
	df['z'] = x['Z']
